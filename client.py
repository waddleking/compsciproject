...
def run_mp_game():
    ...
    current_background = list(color_background)
    ...
    def apply_state(response):
        ...
        game = response["game"]
        pending_events.extend(response.get("events", []))
    ...
    # so n.send() will block here too. fire on a background thread so pygame keeps drawing.
    result_box   = [None]
    fetch_thread = threading.Thread(
        target=lambda: result_box.__setitem__(0, n.send({"type": "get"})),
        daemon=True
    )
    fetch_thread.start()
    ...
    while connected:
                ...
                if result == None:
                    if game.turn_player == player_id:
                        if anim_type == None and end_button.touching():
                            apply_state(n.send({"type": "end_turn"}))
                        ...
                        if selecting == None:
                            if selected_card is not None and selected_card.action_button is not None and selected_card.action_button.touching():
                                ...
                                if selecting == "":
                                    apply_state(n.send({"type": "action_self", "source": selected_source_idx}))
                                ...
                                apply_state(n.send({"type": "retreat", "index": selected_card_idx}))
                                ...
                                for i, card in enumerate(me.active):
                                    if card.touching(mouse) and card.actions > 0:
                                        selected_card = card; selected_card_idx = i
                                ...
                                if player_num != player_id:
                                    ...
                                    for i, card in enumerate(game.players[player_num].active):
                                        if card.touching(mouse) and card.taunt >= highest_taunt:
                                            apply_state(n.send({"type": "action", "source": selected_source_idx, "target_player": player_num, "target": i}))

                                    if game.players[player_num].commander.touching(mouse) and game.players[player_num].commander.taunt >= highest_taunt:
                                        apply_state(n.send({"type": "action", "source": selected_source_idx, "target_player": player_num, "target": None}))
                            ...
                            for i, card in enumerate(me.hand):
                                if card.touching(mouse):
                                    apply_state(n.send({"type": "action_hand", "source": selected_source_idx, "target": i}))
                            selecting = selected_source = selected_source_idx = selected_card = selected_card_idx = None
                        ...
                        for card in me.hand:
                            if len(me.active) < me.max_active or card.spell:
                                if card.touching(mouse) and me.mana >= card.cost and card.actions > 0 and (game.turn - STARTING_PLAYER >= game.num_players or not card.spell):
                                    apply_state(n.send({"type": "play_hand", "index": me.hand.index(card)}))
        ...
        for player in game.players:
            ...
            elif player.commander.hp > 0: player.set_dead(False); player_count += 1
